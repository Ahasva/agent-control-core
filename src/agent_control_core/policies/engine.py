...
def evaluate_plan(
    ...
    if risk.risk_level == RiskLevel.CRITICAL and is_destructive:
        return PolicyDecision(
            decision=PolicyDecisionType.DENY,
            reasons=[
                "Critical-risk destructive actions are denied by default."
            ],
            required_approvals=[],
        )
    ...
    if risk.risk_level == RiskLevel.CRITICAL and touches_credentials:
        return PolicyDecision(
            decision=PolicyDecisionType.DENY,
            reasons=[
                "Critical-risk credential-sensitive actions are denied by default."
            ],
            required_approvals=[],
        )
    ...
    # Additional approval signal from raw task text
