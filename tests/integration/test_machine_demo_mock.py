from agent_control_core.machine.procedures import build_machine_execution_bundle
from agent_control_core.machine_demo import (
    build_machine_demo_scenarios,
    build_machine_demo_state_for_task,
    mock_machine_assess_risk,
    mock_machine_generate_plan,
)
from agent_control_core.policies.engine import evaluate_plan
from agent_control_core.schemas.common import PolicyDecisionType


def test_machine_demo_scenario_1_mock() -> None:
    task = build_machine_demo_scenarios()[0]
    state = build_machine_demo_state_for_task(task)
    plan = mock_machine_generate_plan(task)
    risk = mock_machine_assess_risk(task, plan)
    decision = evaluate_plan(task, plan, risk, state)

    assert decision.decision == PolicyDecisionType.ALLOW

    bundle = build_machine_execution_bundle(task, plan, decision, state)
    assert len(bundle.actions) >= 1


def test_machine_demo_scenario_2_mock() -> None:
    task = build_machine_demo_scenarios()[1]
    state = build_machine_demo_state_for_task(task)
    plan = mock_machine_generate_plan(task)
    risk = mock_machine_assess_risk(task, plan)
    decision = evaluate_plan(task, plan, risk, state)

    assert decision.decision == PolicyDecisionType.REQUIRE_APPROVAL


def test_machine_demo_scenario_3_mock() -> None:
    task = build_machine_demo_scenarios()[2]
    state = build_machine_demo_state_for_task(task)
    plan = mock_machine_generate_plan(task)
    risk = mock_machine_assess_risk(task, plan)
    decision = evaluate_plan(task, plan, risk, state)

    assert decision.decision == PolicyDecisionType.DENY